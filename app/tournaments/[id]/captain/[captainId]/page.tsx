...
export default function CaptainPersonalPage() {
    ...
    async function loadTournamentData() {
      try {
        ...
        if (data) {
          ...
          setHasTeams(!!(data.tournament.teams && data.tournament.teams.length > 0));
        } else {
  ...
  const handleStartDraft = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'ドラフトの開始に失敗しました。');
    } finally {
  ...
  const handleResetDraft = async () => {
    ...
    if (
      !window.confirm('本当にドラフトをリセットしますか？全てのチームと指名履歴が削除されます。')
    ) {
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'ドラフトのリセットに失敗しました。');
    } finally {
  ...
  const handleConfirmNomination = async () => {
    ...
    try {
      ...
      await nominateParticipant(tournament.id, captain.id, selectedParticipant.id);
      ...
      setNominationError(err instanceof Error ? err.message : '指名処理中にエラーが発生しました。');
    } finally {
  ...
  const isParticipantNominated = (participantId: string) => {
    return drafts.some(
      (draft) => draft.participantId === participantId && draft.captainId === captain?.id
    );
  };
  ...
  const getNominatedParticipants = (): NominatedParticipantItem[] => {
    // このキャプテンの指名データ
    const captainNominations = drafts.filter((draft) => draft.captainId === captain?.id);
    ...
    return captainNominations
      .map((draft) => {
        // 対応する参加者を探す
        const nominatedParticipant = participants.find((p) => p.id === draft.participantId);
    ...
    <div className="max-w-4xl mx-auto p-6 md:p-8">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300">キャプテンページ: {captain.name}</p>
        </div>
        <Link
          href={`/tournaments/${params.id}`}
