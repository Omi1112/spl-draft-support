...
describe('TournamentList', () => {
  ...
  it('大会一覧が正常に表示される', async () => {
    ...
    // 各大会名が表示されていることを確認（getAllByTextを使用して複数の要素が存在する可能性に対応）
    expect(screen.getAllByText('テスト大会1')[0]).toBeInTheDocument();
    expect(screen.getAllByText('テスト大会2')[0]).toBeInTheDocument();

    // テーブル行がクリック可能になっていることを確認（role="link"属性の確認）
    // モバイル用とデスクトップ用の両方の表示形式があるため、1大会につき2つのリンク要素が存在する
