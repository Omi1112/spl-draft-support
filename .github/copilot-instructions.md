...
- **GitHub Copilot プロンプト用ルール:** `.github/prompts` フォルダ内の個別のタスク指示書 (`[YYYYMMDDHHMMSS]-[タスクid]-[タスク名]-[タスクの種類].prompt.md`) - **優先度: 低** (ただし、特定のタスク実行時は最優先される場合がある)
...
1.  **実現したい事の確認:** `issues/` フォルダ内の実現したいことを確認します。
2.  **タスク分解:** 実現したい事を、実装可能な単位（通常は1機能）にタスクを分解します。
...
9.  **繰り返し:** 実現したい事の全ての機能が実装されるまで、手順 4〜8 を繰り返します。
...
- **作成ルール:**
  ...
  - ファイル名は以下の命名規則に従います。
    `.github/prompts/[YYYYMMDDHHMMSS]-[タスクid]-[タスク名]-[タスクの種類].prompt.md`
    ...
    - `[タスクの種類]`:
      ...
      - `style`: スタイル調整
      - その他、必要に応じて定義
    - 例: `.github/prompts/20240426101010-123-loginFeature-feat.prompt.md`
- **実行手順:**
  - 実行中のタスクに(`[~]`)を付けてください。
  - タスク実行の時は必ず対応するプロンプトファイルを探して参照してください。
